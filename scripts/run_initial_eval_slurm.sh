#!/usr/bin/env bash
#SBATCH --job-name=cb_initial_eval
...
: "${SPLIT:?Set SPLIT to train, validation, or test}"
: "${RUN:?Set RUN number, e.g. 1}"

CACHE_ROOT="${CACHE_ROOT:-/globalscratch/ucl/cental/sloftus}"
MODEL_DIR="${MODEL//\//_}"
INPUT_JSON="${INPUT_JSON:-data/nli_prompt_json/${SPLIT}-00000-of-00001.json}"
OUTPUT_DIR="${OUTPUT_DIR:-results/${MODEL_DIR}/initial_eval/${SPLIT}/run${RUN}}"
